...
impl MLGraphBuilder {
    pub(crate) fn new_inherited() -> MLGraphBuilder {
        MLGraphBuilder {
            ...
            next_id: Cell::new(0),
            context_accelerated: Cell::new(true),
        }
    }
...
impl MLGraphBuilderMethods<crate::DomTypeHolder> for MLGraphBuilder {
    fn Constructor(
        ...
        let builder = MLGraphBuilder::new(global, can_gc);
        // Store context settings for future use (backend selection, limits)
        builder.context_accelerated.set(context.is_accelerated());
        builder
    }
    ...
    fn Split(
        &self,
        input: &MLOperand,
        splits: UnionTypes::RangeEnforcedUnsignedLongOrRangeEnforcedUnsignedLongSequence,
        ...
        let split_sizes: Vec<u32> = match splits {
            UnionTypes::RangeEnforcedUnsignedLongOrRangeEnforcedUnsignedLongSequence::RangeEnforcedUnsignedLong(n) => {
                let n = n as usize;
                if n == 0 {
            ...
            UnionTypes::RangeEnforcedUnsignedLongOrRangeEnforcedUnsignedLongSequence::RangeEnforcedUnsignedLongSequence(sizes) => {
                sizes
            },
        };
