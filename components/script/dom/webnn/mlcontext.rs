...
use crate::dom::bindings::codegen::Bindings::WebNNBinding::{
    MLContextMethods, MLInputOperandLayout, MLOpSupportLimits, MLOperandDataType,
    MLPowerPreference, MLRankRange, MLSingleInputSupportLimits, MLTensorDescriptor,
    MLTensorLimits,
};
...
impl MLContext {
    pub(crate) fn new_inherited(accelerated: bool, power_preference: MLPowerPreference) -> MLContext {
        MLContext { reflector_: Reflector::new(), accelerated, power_preference }
    }

    pub(crate) fn new(global: &GlobalScope, accelerated: bool, power_preference: MLPowerPreference, can_gc: CanGc) -> DomRoot<MLContext> {
        reflect_dom_object(Box::new(MLContext::new_inherited(accelerated, power_preference)), global, can_gc)
    }

    pub(crate) fn is_accelerated(&self) -> bool { self.accelerated }
}
